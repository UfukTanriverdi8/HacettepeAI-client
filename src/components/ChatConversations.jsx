  ...
  return (
    ...
    <div ref={chatContainerRef} className='flex flex-col items-center p-4 overflow-y-auto h-full scroll-container'>
      ...
      {chatHistory.map((chat, index) => (
        <ChatMessage key={index} sender={chat.sender} message={chat.message} isPlaceholder={chat.isPlaceholder} skipTypewriter={chat.skipTypewriter} timestamp={chat.timestamp} question={chat.question} language={language} />
      ))}
    </div>
  );
};
